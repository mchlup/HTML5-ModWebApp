...
export function renderOrders(container, { labels, onCountChange } = {}) {
  ...
  if (!orders.length) {
    ...
    table.className = 'striped production-table';
    ...
    sortedOrders.forEach((order) => {
      ...
      row.innerHTML = `
        ...
        <td>${order.dueDate || '—'}</td>
        <td>${order.status}</td>
        <td class="form-actions"><button type="button" class="danger production-btn production-btn-danger production-btn-sm">${labels.delete}</button></td>
      `;

...
try {
    ...
    switch ($method) {
        case 'GET':
            productionMaterialsHandleGet($pdo);
            break;
        ...
        case 'POST':
            ...
            requireCsrfToken();
            productionMaterialsHandlePost($pdo);
            break;
        ...
        case 'DELETE':
            ...
            requireCsrfToken();
            productionMaterialsHandleDelete($pdo);
            break;
...
/**
 * GET /modules/production/api/materials.php
 * GET /modules/production/api/materials.php?id=123
 */
function productionMaterialsHandleGet($pdo)
{
    ...
    if ($id > 0) {
        ...
        jsonResponse(array(
            'success'  => true,
            'material' => productionMaterialsMapRow($row),
        ));
    }
    ...
    $countSql = 'SELECT COUNT(*) FROM production_materials ' . $where;
    ...
    $sql = 'SELECT id, code, name, supplier, price, density, solids, okp, oil, voc, safety, note, created_at
            FROM production_materials ' . $where . ' ORDER BY ' . $orderBy . ' ' . $sortDir . ' LIMIT :limit OFFSET :offset';
    $stmt = $pdo->prepare($sql);
    foreach ($params as $key => $val) {
    ...
    $materials = array();
    foreach ($rows as $row) {
        $materials[] = productionMaterialsMapRow($row);
    }
...
/**
 * POST /modules/production/api/materials.php
 * JSON body – insert/update podle existence "id"
 */
function productionMaterialsHandlePost($pdo)
{
    ...
    if ($id > 0) {
        ...
        $sql = 'INSERT INTO production_materials
...
/**
 * DELETE /modules/production/api/materials.php?id=123
 */
function productionMaterialsHandleDelete($pdo)
{
    $id = isset($_GET['id']) ? (int) $_GET['id'] : 0;
    if ($id <= 0) {
        jsonResponse(array(
    ...
    $stmt = $pdo->prepare('DELETE FROM production_materials WHERE id = :id');
    $stmt->execute(array(':id' => $id));
 ...
 */
function productionMaterialsMapRow($row)
{
    return array(
